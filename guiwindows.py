...
class PageThree(tk.Frame):
    def __init__(self, parent, controller): 
        ...
        self.buy_button.place(x=self.winfo_screenwidth()/5+10,y=100)
        self.backtest_button = tk.Button(self,text="Testuj strategie", command=lambda: use_strategy(self.market,self.strategy,False))
        self.backtest_button.place(x=self.winfo_screenwidth()/5+60,y=100)
        ...
        self.algotrade_button.place(x=self.winfo_screenwidth()/5+180,y=100)
    def on_select(self,event):
        print(type(event.widget.get('active')))
